...
[`predict()`](https://www.rdocumentation.org/packages/xgboost/versions/0.71.2/topics/predict.xgb.Booster)で訓練データのインスタンスが所属するLeafのノード番号を取得する。
...
ただし、取得されたのち、この情報が使われている形跡が見当たらなかった。

## 各roundの予測ルールを抽出・整理する
`xgboostExplainer:::getStatsForTrees()`により処理される。
...
以下では、`xgboostExplainer:::getStatsForTrees()`の一連のステップをトレースする((関数内部でのデータの更新が、前提としてdata.table::copyによる`- attr(*, ".internal.selfref")=<externalptr>`で、参照渡しが行われているのに注意))
